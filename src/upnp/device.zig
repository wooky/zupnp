...
pub fn init(
    ...
    return .{
        ...
        .udn = "udn:TODO",
        .udn_url = try std.fmt.allocPrint("{}/{}", .{upnp_server.base_url, "udn:TODO"}),
    };
}
...
pub fn start(self: *Device) !void {
    ...
    if (c.UpnpRegisterRootDevice2(c.UPNPREG_BUF_DESC, schema, schema.len, 0, onEvent, self, &self.handle) != c.UPNP_E_SUCCESS) return Error;
    if (c.UpnpSendAdvertisement(handle, 100) != c.UPNP_E_SUCCESS) return Error;
}
...
fn createSchema(self: *Device) ![:0]const u8 {
    ...
    while (iter.next()) |kv| {
        try schema_services.append(.{
            ...
            .SCPDURL = try std.fmt.allocPrint("{}/{}/{}", .{self.udn_url, kv.value.id, "IDK"}),
            .controlURL = try std.fmt.allocPrint("{}/{}/{}", .{self.udn_url, kv.value.id, "control"}),
            .eventSubURL = try std.fmt.allocPrint("{}/{}/{}", .{self.udn_url, kv.value.id, "event"}),
        });
    }
...
fn handleAction(self: *Device, action: *c.UpnpActionRequest) !void {
    const service_id = cStringToSlice(c.UpnpActionRequest_get_ServiceID(action));
